...
function handleSuccess(id: number) {
  toast.add({
    severity: 'success',
    summary: 'Erfolg',
    detail: 'Qualifikation wurde aktualisiert.',
    life: 3000
  })
...
function handleCancel() {
  if (qualificationId.value) {
    router.push(`/qualifications/${qualificationId.value}`)
  } else {
...
<template>
  <div class="edit-view">
    <div class="view-header">
      <div>
        <h1 class="title">Qualifikation bearbeiten</h1>
        <p class="subtitle">Passe die Details dieser Qualifikation an.</p>
      </div>
      <Button
        label="Zurück"
    ...
    <AttachmentsSection
      v-if="qualificationId"
      :source-id="qualificationId"
      source-type="qualification"
    />

    <div v-else class="invalid-state">
      <i class="pi pi-exclamation-triangle"></i>
      <p>Die Qualifikation konnte nicht gefunden werden.</p>
