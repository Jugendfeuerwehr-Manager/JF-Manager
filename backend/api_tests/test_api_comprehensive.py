...
class BaseAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.regular_user = User.objects.create_user(
            username="user", email="user@test.com", password="user123!", first_name="Test", last_name="User"
        )
    ...
    def authenticate_user(self, user, password="user123!"):
        ...
        if user.username == "admin" or user.username == "authorized":
            pass

        self.client.force_authenticate(user=user)

    def grant_permissions(self, user, model_name, permissions=None):
...
class UserAPITests(BaseAPITestCase):
    ...
    def test_user_me_update(self):
        ...
        update_data = {"first_name": "Updated", "last_name": "Name", "email": "updated@test.com"}
...
class PermissionsTests(BaseAPITestCase):
    ...
    def test_add_permission_required_for_create(self):
        ...
        new_item = {"name": "Test Item", "category": 1}
...
class CRUDOperationsTests(BaseAPITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.grant_permissions(self.authorized_user, "category", ["view", "add", "change", "delete"])
        self.authenticate_user(self.authorized_user)
    ...
    def test_update_resource(self):
        ...
        if create_response.status_code == 201:
            ...
            response = self.client.patch(f"/api/v1/inventory/categories/{category_id}/", update_data)
            if response.status_code == 200:
