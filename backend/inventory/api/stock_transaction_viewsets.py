...
class TransactionViewSet(BasePermissionedViewSet, viewsets.ModelViewSet):
    ...
    def discard_statistics(self, request):
        ...
        by_reason = list(
            discard_qs.values('discard_reason').annotate(
                count=Count('id'), total_quantity=Sum('quantity')
            ).order_by('-total_quantity')
        )

        by_category_raw = discard_qs.select_related(
            'item__category', 'item_variant__parent_item__category'
        ).values(
            'item__category__name', 'item_variant__parent_item__category__name'
        ).annotate(count=Count('id'), total_quantity=Sum('quantity')).order_by('-total_quantity')
