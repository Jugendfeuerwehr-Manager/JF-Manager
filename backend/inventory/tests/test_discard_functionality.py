...
class DiscardReasonFieldTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_superuser(username="admin", email="admin@test.com", password="testpass123")
        self.category = Category.objects.create(name="Test Category", schema={})
    ...
    def test_discard_transaction_requires_reason(self):
        ...
        transaction = Transaction(
            transaction_type="DISCARD", item=self.item, source=self.storage_location, quantity=5, user=self.user
        )
...
class DiscardFilteringTestCase(TestCase):
    ...
    def test_filter_transactions_by_discard_reason(self):
        ...
        # Filter by LOST
        response = self.client.get("/api/v1/inventory/transactions/", {"discard_reason": "LOST"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        ...
        # Filter by DAMAGED
        response = self.client.get("/api/v1/inventory/transactions/", {"discard_reason": "DAMAGED"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertEqual(data["count"], 1)
