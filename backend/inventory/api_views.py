...
from .models import Category, Item, ItemVariant, Stock, StorageLocation
...
def search_items(request):
    ...
    if location_id:
        items = items.filter(stock__location_id=location_id, stock__quantity__gt=0).distinct()
    ...
    for item in page_obj:
        ...
        if location_id:
            try:
                ...
                stock_info = {"quantity": stock.quantity, "location_name": stock.location.name}
            except Stock.DoesNotExist:
                stock_info = {"quantity": 0, "location_name": ""}
        else:
            # Gesamtbestand über alle Lagerorte
            total_stock = Stock.objects.filter(item=item).aggregate(total=Sum("quantity"))["total"] or 0
    ...
    return _deprecated(
        JsonResponse({"results": results, "has_more": page_obj.has_next(), "total_count": paginator.count})
    )
...
def search_locations(request):
    ...
    # Nach Text suchen
    locations = locations.filter(Q(name__icontains=query) | Q(parent__name__icontains=query))
    ...
    for location in page_obj:
        ...
        total_items = Stock.objects.filter(location=location).aggregate(total=Sum("quantity"))["total"] or 0
        ...
        results.append(
            {
                ...
                "stock_info": {"unique_items": stock_count, "total_quantity": total_items},
            }
        )
...
def search_members(request):
    ...
    members = members.filter(Q(name__icontains=query) | Q(lastname__icontains=query))
    ...
    for member in members_page:
        ...
        results.append({"id": member.id, "text": full_name, "detail": detail})

    return _deprecated(JsonResponse({"results": results, "has_more": has_more}))
    ...
    if location_id:
        ...
        try:
            ...
            try:
                ...
                stock_data = {"location_name": location.get_full_path(), "quantity": 0, "has_stock": False}
        ...
        stock_data = {
            ...
            "locations": [
                {"location_name": stock.location.get_full_path(), "quantity": stock.quantity}
                for stock in stocks
                if stock.quantity > 0
            ],
        }
...
def search_categories(request):
    ...
    categories = Category.objects.filter(name__icontains=query).order_by("name")[:20]

    results = [
        {"id": category.id, "name": category.name, "item_count": category.items.count()} for category in categories
    ]
...
def get_category_schema(request, category_id):
    ...
    return _deprecated(JsonResponse({"category": {"id": category.id, "name": category.name}, "schema": schema}))
...
def get_items_by_category(request, category_id):
    ...
    return _deprecated(JsonResponse({"category": {"id": category.id, "name": category.name}, "items": results}))
...
def get_item_details(request, item_id):
    ...
    if item.category:
        result["category"] = {"id": item.category.id, "name": item.category.name, "schema": item.category.schema or {}}

    return _deprecated(JsonResponse(result))


@method_decorator(
    [login_required, permission_required("inventory.view_itemvariant", raise_exception=True)], name="dispatch"
)
class ItemVariantAPIView(View):
    ...
    def post(self, request):
        ...
        variant = ItemVariant.objects.create(
            parent_item=parent_item, sku=data.get("sku", ""), variant_attributes=data.get("variant_attributes", {})
        )
    ...
    def delete(self, request, variant_id):
        ...
        if total_stock > 0:
            return JsonResponse(
                {"error": f"Cannot delete variant with existing stock ({total_stock} items)"}, status=400
            )
...
def search_items_and_variants(request):
    ...
    for variant in variants:
        ...
        if location_id:
            ...
            total_stock = Stock.objects.filter(item_variant=variant).aggregate(total=Sum("quantity"))["total"] or 0
            stock_info = {"total_quantity": total_stock}
    ...
    return _deprecated(
        JsonResponse({"results": list(page_obj), "has_more": page_obj.has_next(), "total_count": len(results)})
    )
...
def get_recent_items(request):
    ...
    return _deprecated(JsonResponse({"results": results, "has_more": False, "total_count": len(results)}))
