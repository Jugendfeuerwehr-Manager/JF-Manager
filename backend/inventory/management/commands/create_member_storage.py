    ...
    def handle(self, *args, **options):
        ...
        if parent_id:
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f"Übergeordneter Lagerort: {parent_location.name}"))
            except StorageLocation.DoesNotExist:
                self.stdout.write(self.style.ERROR(f"Lagerort mit ID {parent_id} nicht gefunden"))
                return

        # Finde Mitglieder ohne Lagerplatz
        members_without_storage = Member.objects.filter(personal_storage_location__isnull=True).order_by(
            "lastname", "name"
        )

        if not members_without_storage.exists():
            self.stdout.write(self.style.SUCCESS("Alle Mitglieder haben bereits einen Lagerplatz."))
            return

        self.stdout.write(f"Gefunden: {members_without_storage.count()} Mitglieder ohne Lagerplatz")
            ...
            for member in members_without_storage:
                ...
                try:
                    storage_location = StorageLocation.objects.create(
                        name=storage_name, parent=parent_location, is_member=True, member=member
                    )
                    created_count += 1
                    self.stdout.write(self.style.SUCCESS(f"✓ Erstellt: {storage_location.name}"))
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"✗ Fehler bei {storage_name}: {e!s}"))
