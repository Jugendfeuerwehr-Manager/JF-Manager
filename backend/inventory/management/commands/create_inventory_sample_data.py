    ...
    def handle(self, *args, **options):
        if options["clear"]:
            ...
            Category.objects.filter(
                name__in=["Uniformen", "Ausrüstung", "Fahrzeuge", "Werkzeug", "Erste Hilfe"]
            ).delete()

        # Kategorien erstellen
        self.stdout.write("Erstelle Kategorien...")
        categories = {
            "Uniformen": {"größe": "string", "farbe": "string", "material": "string"},
            "Ausrüstung": {"typ": "string", "material": "string", "gewicht": "number"},
            "Fahrzeuge": {"kennzeichen": "string", "baujahr": "number", "hersteller": "string"},
            "Werkzeug": {"typ": "string", "zustand": "string"},
            "Erste Hilfe": {"ablaufdatum": "date", "hersteller": "string"},
        }

        category_objects = {}
        for cat_name, schema in categories.items():
            cat, created = Category.objects.get_or_create(name=cat_name, defaults={"schema": schema})
        ...
        locations = ["Hauptlager", "Fahrzeughalle", "Jugendschrank", "Erste-Hilfe-Raum", "Werkstatt"]

        location_objects = {}
        for loc_name in locations:
            loc, created = StorageLocation.objects.get_or_create(name=loc_name, defaults={"is_member": False})
        ...
        for member in members:
            loc, created = StorageLocation.objects.get_or_create(
                name=f"{member.name} {member.lastname}", defaults={"is_member": True, "member": member}
            )
            if created:
        ...
        self.stdout.write(self.style.SUCCESS("Beispieldaten erfolgreich erstellt!"))
        self.stdout.write("Sie können nun das Inventarsystem unter /inventory/ aufrufen.")
