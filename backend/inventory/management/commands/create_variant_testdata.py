...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        clothing_category, created = Category.objects.get_or_create(
            name="Kleidung", defaults={"schema": {"größe": "string", "farbe": "string", "material": "string"}}
        )
        if created:
            self.stdout.write(f'  ✓ Kategorie "{clothing_category.name}" erstellt')

        equipment_category, created = Category.objects.get_or_create(
            name="Ausrüstung", defaults={"schema": {"größe": "string", "gewicht": "number", "typ": "string"}}
        )
        if created:
        ...
        main_storage, created = StorageLocation.objects.get_or_create(name="Hauptlager", defaults={"is_member": False})
        if created:
            self.stdout.write(f'  ✓ Lagerort "{main_storage.name}" erstellt')

        clothing_storage, created = StorageLocation.objects.get_or_create(
            name="Kleiderkammer", defaults={"parent": main_storage, "is_member": False}
        )
        if created:
        ...
        pants_item, created = Item.objects.get_or_create(
            ...
            defaults={
                ...
                "attributes": {"material": "Nomex", "farbe": "schwarz"},
            },
        )
        if created:
        ...
        for size in pants_sizes:
            variant, created = ItemVariant.objects.get_or_create(
                parent_item=pants_item, variant_attributes={"größe": size}, defaults={"sku": f"FW-HOSE-{size}"}
            )
            if created:
        ...
        for size in jacket_sizes:
            variant, created = ItemVariant.objects.get_or_create(
                parent_item=jacket_item, variant_attributes={"größe": size}, defaults={"sku": f"FW-JACKE-{size}"}
            )
            if created:
                self.stdout.write(f'    ✓ Variante "Größe {size}" erstellt')
        ...
        for size in mask_sizes:
            variant, created = ItemVariant.objects.get_or_create(
                parent_item=mask_item, variant_attributes={"größe": size}, defaults={"sku": f"AS-MASKE-{size}"}
            )
            if created:
                self.stdout.write(f'    ✓ Variante "Größe {size}" erstellt')
        ...
        normal_item, created = Item.objects.get_or_create(
            ...
            defaults={
                ...
                "attributes": {"typ": "Schaum", "gewicht": 6},
            },
        )
        if created:
