...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dup in duplicates:
            ...
            self.stdout.write(f'Found {count} duplicate records for {person_name} in service "{service_topic}"')
            ...
            for record in records_to_delete:
                if dry_run:
                    self.stdout.write(f"  [DRY RUN] Would delete: ID={record.id}, State={record.state}")
                else:
                    record_id = record.id
                    record.delete()
                    self.stdout.write(f"  Deleted: ID={record_id}, State={record.state}")
                total_deleted += 1
