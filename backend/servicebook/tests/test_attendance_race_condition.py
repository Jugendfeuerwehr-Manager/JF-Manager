...
class AttendanceRaceConditionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client.force_authenticate(user=self.user)

        # Create test members
        self.member1 = Member.objects.create(name="John", lastname="Doe")
        self.member2 = Member.objects.create(name="Jane", lastname="Smith")

        # Create test service
        start_time = datetime.now()
        self.service = Service.objects.create(
            start=start_time, end=start_time + timedelta(hours=2), topic="Test Service", place="Test Location"
        )
    ...
    def test_unique_constraint_prevents_duplicates(self):
        ...
        Attendance.objects.create(person=self.member1, service=self.service, state="A")

        # Attempting to create duplicate should raise IntegrityError
        from django.db import IntegrityError

        with self.assertRaises(IntegrityError):
            Attendance.objects.create(person=self.member1, service=self.service, state="E")
    ...
    def test_bulk_update_with_delete(self):
        ...
        # Create initial attendance
        Attendance.objects.create(person=self.member1, service=self.service, state="A")
        Attendance.objects.create(person=self.member2, service=self.service, state="E")
        ...
        self.assertFalse(Attendance.objects.filter(person=self.member1, service=self.service).exists())

        # Verify member2 attendance still exists
        self.assertTrue(Attendance.objects.filter(person=self.member2, service=self.service).exists())
