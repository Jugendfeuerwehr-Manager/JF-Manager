...
class OrderableItemViewSet(viewsets.ModelViewSet):
    ...
    def categories(self, request):
        ...
        categories = OrderableItem.objects.values_list("category", flat=True).distinct().order_by("category")
    ...
    def sizes(self, request, pk=None):
        ...
        return Response({"has_sizes": item.has_sizes, "sizes": item.get_sizes_list() if item.has_sizes else []})
    ...
    def popular(self, request):
        ...
        popular_items = (
            OrderableItem.objects.filter(is_active=True)
    ...
    def by_category(self, request):
        ...
        items = self.queryset.filter(is_active=True, category__iexact=category)
