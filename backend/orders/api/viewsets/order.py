...
class OrderViewSet(DepartmentScopeViewSetMixin, viewsets.ModelViewSet):
    """
    ...
    permission_classes = [permissions.IsAuthenticated, DepartmentRoleModelPermissions, CanManageOrders]
    ...
    def get_queryset(self):
        """Optimize queryset with prefetch, then apply department scope."""
        queryset = Order.objects.select_related(
            "member",
            "member__group",
            "ordered_by",
            "department",
        ).prefetch_related("items__item", "items__status")
    ...
    def detail_with_history(self, request, pk=None):
        ...
        for item_data in data["items"]:
            ...
            item_data["history"] = OrderItemStatusHistorySerializer(history, many=True).data
    ...
    def statistics(self, request):
        ...
        # Member statistics
        member_stats = (
            queryset.values("member__id", "member__name", "member__lastname")
        ...
        for i in range(12):
            ...
            count = queryset.filter(order_date__date__gte=month_start, order_date__date__lte=month_end).count()

            monthly_data.append({"month": month_start.strftime("%Y-%m"), "count": count})
        ...
        return Response(
            {
                ...
                "pending_items": OrderItem.objects.filter(
                    order__in=queryset, status__code__in=["NEW", "ORDERED"]
                ).count(),
                "delivered_items": OrderItem.objects.filter(order__in=queryset, status__code="DELIVERED").count(),
            }
        )
    ...
    def recent(self, request):
        ...
        recent_orders = self.get_queryset().order_by("-order_date")[:limit]
    ...
    def pending(self, request):
        ...
        pending_orders = self.get_queryset().exclude(items__status__pk__in=exclude_statuses).distinct()
    ...
    def by_member(self, request):
        ...
        orders = self.get_queryset().filter(member__id=member_id).order_by("-order_date")
    ...
    def send_summary(self, request):
        ...
        try:
            ...
            return Response({"error": "NEW status not found in system"}, status=status.HTTP_400_BAD_REQUEST)

        # Build queryset - only orders with NEW items
        orders_queryset = Order.objects.filter(items__status=new_status).distinct().order_by("-order_date")

        # Check if there are any NEW orders
        if not orders_queryset.exists():
            return Response({"error": "Keine neuen Bestellungen gefunden"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            success = OrderNotificationService.send_order_summary_notification(
                recipient_email=recipient_email, orders=orders_queryset, filters=filters, request=request
            )
            ...
            if success:
                ...
                return Response(
                    {"error": "Fehler beim Versenden der E-Mail"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
