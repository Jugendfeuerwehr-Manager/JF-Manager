...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pending_statuses = OrderStatus.objects.filter(code__in=pending_status_codes, is_active=True)

        # Find orders with pending items older than the threshold
        orders_with_pending = Order.objects.filter(
            order_date__lt=cutoff_date, items__status__in=pending_statuses
        ).distinct()
        ...
        for order in orders_with_pending:
            ...
            if pending_items.exists():
                self.stdout.write(
                    f"Order #{order.pk} for {order.member.get_full_name()} has {pending_items.count()} pending items"
                )

                if not dry_run:
                    try:
                        success = OrderNotificationService.send_pending_order_reminder(order, pending_items)
                        if success:
                            emails_sent += 1
                            self.stdout.write(self.style.SUCCESS(f"  ✓ Reminder sent for order #{order.pk}"))
                        else:
                            self.stdout.write(self.style.ERROR(f"  ✗ Failed to send reminder for order #{order.pk}"))
                    except Exception as e:
                        self.stdout.write(self.style.ERROR(f"  ✗ Error sending reminder for order #{order.pk}: {e}"))
                else:
                    self.stdout.write("  (dry run - no email sent)")

        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"Dry run completed. Would have sent {orders_with_pending.count()} reminder emails.")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
