...
class Command(BaseCommand):
    ...
    def load_template_content(self, template_name, template_type="html"):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"Template file not found: {template_path}"))
            return ""
        ...
        for template_data in templates:
            ...
            if created:
                ...
                if updated:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Template already exists: {template.name}"))

        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully processed {len(templates)} templates ({created_count} created, {updated_count} updated)"
            )
        )
