...
class EmailTestForm(forms.Form):
    recipient = forms.EmailField(
        label="Empfänger E-Mail", help_text="E-Mail-Adresse an die eine Test-E-Mail gesendet werden soll"
    )
    subject = forms.CharField(label="Betreff", initial="Test E-Mail vom JF-Manager")
    message = forms.CharField(
        label="Nachricht", widget=forms.Textarea, initial="Dies ist eine Test-E-Mail vom JF-Manager System."
    )
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.error(
                        request,
                        f"Fehler beim Senden der Test-E-Mail: {e!s}. Bitte überprüfen Sie die SMTP-Einstellungen.",
                    )
        else:
            form = EmailTestForm()
