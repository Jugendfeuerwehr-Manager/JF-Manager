...
def run_tests(test_path="api_tests", verbosity=1, with_coverage=False):
    ...
    if with_coverage:
        ...
        cmd = ["coverage", "run", "--source=.", "manage.py", "test", test_path, f"--verbosity={verbosity}"]
        subprocess.run(cmd)
...
def main():
    ...
    parser.add_argument("--users", action="store_true", help="Run user API tests only")
    parser.add_argument("--members", action="store_true", help="Run member API tests only")
    parser.add_argument("--inventory", action="store_true", help="Run inventory API tests only")
    parser.add_argument("--orders", action="store_true", help="Run orders API tests only")
    parser.add_argument("--qualifications", action="store_true", help="Run qualifications API tests only")
    parser.add_argument("--crud", action="store_true", help="Run CRUD operation tests only")
