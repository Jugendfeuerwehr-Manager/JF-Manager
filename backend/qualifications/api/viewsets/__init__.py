...
class QualificationViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['member__first_name', 'member__last_name', 'user__first_name', 'user__last_name', 'type__name', 'issued_by']
    ...
    def statistics(self, request):
        ...
        expiring = qualifications_qs.filter(
            date_expires__gte=today,
            date_expires__lte=soon_threshold
        )
        active_tasks = special_tasks_qs.filter(
    ...
    def calculate_expiry(self, request):
        ...
        if not type_id or not date_acquired_str:
            return Response(
                {'error': 'type_id and date_acquired are required'},
