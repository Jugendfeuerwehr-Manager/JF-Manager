...
class QualificationTypeModelTest(TestCase):
    def test_create_qualification_type(self):
        ...
        qual_type = QualificationType.objects.create(
            name="Grundlehrgang", expires=True, validity_period=24, description="Grundausbildung für Jugendfeuerwehr"
        )
    ...
    def test_qualification_type_validation(self):
        ...
        qual_type = QualificationType(name="Test", expires=True, validity_period=None)
    ...
    def test_qualification_type_no_expiry(self):
        ...
        qual_type = QualificationType.objects.create(name="Lifetime Certificate", expires=False)
...
class QualificationModelTest(TestCase):
    def setUp(self):
        ...
        self.member = Member.objects.create(name="Max", lastname="Mustermann", group=group, status=status)

        # Qualifikationstypen
        self.expiring_type = QualificationType.objects.create(name="Sprechfunk", expires=True, validity_period=24)

        self.permanent_type = QualificationType.objects.create(name="Grundlehrgang", expires=False)

    def test_create_qualification_for_user(self):
        """Test der Erstellung einer Qualifikation für einen User"""
        qualification = Qualification.objects.create(
            type=self.expiring_type, user=self.user, date_acquired=date.today(), issued_by="Feuerwehrschule"
        )
    ...
    def test_create_qualification_for_member(self):
        ...
        qualification = Qualification.objects.create(
            type=self.permanent_type, member=self.member, date_acquired=date.today()
        )
    ...
    def test_automatic_expiry_date_calculation(self):
        ...
        qualification = Qualification.objects.create(
            type=self.expiring_type, user=self.user, date_acquired=acquired_date
        )
    ...
    def test_qualification_validation(self):
        ...
        qualification = Qualification(type=self.permanent_type, date_acquired=date.today())
        ...
        qualification = Qualification(
            type=self.permanent_type, user=self.user, member=self.member, date_acquired=date.today()
        )
    ...
    def test_qualification_string_representation(self):
        ...
        qualification = Qualification.objects.create(
            type=self.permanent_type, user=self.user, date_acquired=date.today()
        )
