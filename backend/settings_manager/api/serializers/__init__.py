...
class EmailSettingsSerializer(serializers.Serializer):
    ...
    email_use_tls = serializers.BooleanField(
        required=False,
        help_text='Use TLS encryption for email connection'
    )
    ...
    def validate(self, data):
        ...
        if data.get('email_use_tls') and data.get('email_use_ssl'):
            raise serializers.ValidationError(
                'TLS and SSL cannot be enabled at the same time'
            )
        return data
...
class MemberSettingsSerializer(serializers.Serializer):
    ...
    alert_threshold = serializers.IntegerField(
        required=False,
        min_value=1,
        help_text='Threshold for absence alert (number of absences)'
    )
    alert_threshold_last_entries = serializers.IntegerField(
        required=False,
        min_value=1,
        help_text='Number of recent services to check for absences'
    )


class ServiceSettingsSerializer(serializers.Serializer):
    """Serializer for service settings"""
    service_start_time = serializers.TimeField(
        required=False,
        help_text='Default start time for new services'
    )
    service_end_time = serializers.TimeField(
        required=False,
        help_text='Default end time for new services'
    )
...
class CategorySettingsUpdateSerializer(serializers.Serializer):
    ...
    settings = serializers.DictField(
        required=True,
        help_text='Settings key-value pairs to update'
    )
