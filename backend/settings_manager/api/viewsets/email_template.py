...
class EmailTemplateViewSet(viewsets.ModelViewSet):
    ...
    TEMPLATE_VARIABLES = {
        ...
        "bulk_update": {
            ...
            "sample_data": {
                "order": {"pk": 42},
                "member": {"first_name": "Max", "last_name": "Mustermann"},
                "updated_items": [{"name": "T-Shirt", "quantity": 2}, {"name": "Hose", "quantity": 1}],
                "new_status": {"name": "Geliefert", "color": "green"},
    ...
    @extend_schema(summary="Get template types", description="Get list of available template types")
    @action(detail=False, methods=["get"])
    ...
    def default_content(self, request):
        ...
        if not template_type:
            return Response({"error": "template_type parameter is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if template_type not in template_files:
            return Response(
                {"error": f"No default template file for type: {template_type}"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        try:
            ...
            return Response(
                {"error": f"Template file not found: {template_files[template_type]}"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            logger.error(f"Failed to load default template: {e}")
            return Response({"error": f"Failed to load template: {e!s}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
