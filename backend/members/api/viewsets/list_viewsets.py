...
class MemberListViewSet(viewsets.ModelViewSet):
    ...
    def remove_member(self, request, pk=None):
        ...
        member_list = self.get_object()
        member_id = request.data.get('member_id')
        deleted, _ = MemberListEntry.objects.filter(
            member_list=member_list, member_id=member_id
        ).delete()
    ...
    def bulk_add(self, request, pk=None):
        ...
        for member_id in member_ids:
            try:
                member = Member.objects.get(pk=member_id)
                _, created = MemberListEntry.objects.get_or_create(
                    member_list=member_list, member=member
                )
                if created:
                    added += 1
            except Member.DoesNotExist:
    ...
    def update_entry_notes(self, request, pk=None):
        ...
        updated = MemberListEntry.objects.filter(
            member_list=member_list, member_id=member_id
        ).update(notes=notes)
        if not updated:
