                ...
                for perm_type in permission_types:
                    try:
                        permission = Permission.objects.get(
                            codename=f"{perm_type}_{model_name}", content_type__app_label=app_label
                        )
                        user.user_permissions.add(permission)
                        granted_count += 1
                    except Permission.DoesNotExist:
                        self.stdout.write(
                            self.style.WARNING(f"  Permission {perm_type}_{model_name} not found for {app_label}")
                        )

            user.save()
            self.stdout.write(
                self.style.SUCCESS(f"  Successfully granted {granted_count} permissions to {user.username}")
            )
