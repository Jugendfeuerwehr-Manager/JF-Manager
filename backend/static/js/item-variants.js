...
class ItemVariantManager {
    ...
    async loadItemVariants() {
        ...
        try {
            // Ensure itemId is numeric to avoid malformed URL errors (e.g. empty string -> '//variants')
            if (!/^[0-9]+$/.test(String(itemId))) {
                console.warn('Invalid item id for variants fetch:', itemId);
                return;
            }
            const response = await fetch(`/inventory/api/items/${itemId}/variants/`, {credentials: 'same-origin'});
            if (!response.ok) {
                console.error('Variants fetch failed', response.status);
                return;
            }
            const data = await response.json();
            if (!data || typeof data !== 'object') {
                console.error('Unexpected variants response format', data);
                return;
            }
    ...
    showVariantForm(variantId = null) {
        ...
        if (window.bootstrap && window.bootstrap.Modal) {
            // Re-use existing instance if present
            let modalInstance = window.bootstrap.Modal.getOrCreateInstance ? window.bootstrap.Modal.getOrCreateInstance(modal) : null;
            if (!modalInstance) {
                try {
                    modalInstance = new bootstrap.Modal(modal);
                } catch (e) {
                    console.warn('Bootstrap Modal init failed, fallback to display block', e);
                }
            }
        ...
        if (modal) {
            if (window.bootstrap && window.bootstrap.Modal) {
                ...
                if (window.bootstrap.Modal.getInstance) {
                    modalInstance = window.bootstrap.Modal.getInstance(modal);
                }
                if (!modalInstance && window.bootstrap.Modal.getOrCreateInstance) {
