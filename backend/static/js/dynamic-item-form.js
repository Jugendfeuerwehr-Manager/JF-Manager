...
class DynamicItemForm {
    ...
    this.lastRenderedCategory = null;
    this.renderInProgress = false;
    this.debouncedLoad = this.debounce(() => this.loadCategorySchema(), 50);
    this.loadCategorySchema(); // initial
    }
    ...
    async loadCategorySchema() {
        ...
        // Abort if same category already rendered
        if (this.lastRenderedCategory === categoryId && dynamicContainer.dataset.rendered === '1') {
            return;
        }
        // Prepare for fresh render
        dynamicContainer.innerHTML = '';
        
        if (!categoryId) {
            dynamicContainer.dataset.rendered = '0';
            return;
        }
        ...
        try {
            ...
            if (data.schema && Object.keys(data.schema).length > 0) {
                // Guard: mark rendering in progress
                dynamicContainer.dataset.rendered = '0';
                this.renderDynamicFields(data.schema, dynamicContainer, attributesField);
                this.lastRenderedCategory = categoryId;
                dynamicContainer.dataset.rendered = '1';
            } else {
                dynamicContainer.dataset.rendered = '0';
            }
        } catch (error) {
    ...
    container.appendChild(title);
        ...
        Object.entries(schema).forEach(([fieldName, fieldTypeRaw]) => {
            // Skip null/undefined field definitions
            if (fieldTypeRaw === null || fieldTypeRaw === undefined) {
                return;
            }
            // Support object definitions in future (e.g. {type: "string"})
            let fieldType = fieldTypeRaw;
            if (typeof fieldTypeRaw === 'object') {
                fieldType = fieldTypeRaw.type || 'text';
            }
            try {
                const existingVal = existingAttributes ? existingAttributes[fieldName] : undefined;
                const fieldGroup = this.createDynamicField(fieldName, String(fieldType), existingVal);
                container.appendChild(fieldGroup);
            } catch (e) {
                console.warn('Failed to render dynamic field', fieldName, e);
            }
        });
...
DynamicItemForm.prototype.debounce = function(fn, delay) {
    let timer = null;
    return function() {
        const context = this;
